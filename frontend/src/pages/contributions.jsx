...
import { useListContributions, useUpsertMyContribution, getListContributionsQueryKey, getGetDashboardSummaryQueryKey } from "@/lib/api.js";
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto w-full pb-24 md:pb-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-2 text-lg">Who's putting in what before the next invoice uprising.</p>
      </div>

      {/* Hero Stats */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
        <Card className="bg-card/90 border-secondary/40 shadow-[6px_6px_0_hsl(var(--secondary))] relative overflow-hidden rounded-lg">
          <CardContent className="p-8 md:p-12 text-center">
            ...
            {!isEditing ? (
              <Button 
                ...
                className="bg-secondary text-secondary-foreground hover:bg-secondary/80 rounded-md px-8 h-12 text-base font-black shadow-lg"
              >
                {myContribution ? "Update My Contribution" : "Add My Contribution"}
              </Button>
            ) : (
              <form onSubmit={handleSave} className="max-w-sm mx-auto flex gap-2 mt-4 bg-background/50 p-2 rounded-md border border-border backdrop-blur-sm">
      ...
      {/* List */}
      <div className="space-y-4">
        <h3 className="text-xl font-black px-2">The Roster</h3>
        {contributions && contributions.length > 0 ? (
          <div className="grid gap-4">
            {contributions.map((c, i) => (
              ...
              >
                <Card className="border-border bg-card/80 backdrop-blur-sm hover:bg-card transition-colors rounded-lg">
          ...
          <div className="text-center p-12 bg-card/80 rounded-lg border border-dashed border-border text-muted-foreground">
