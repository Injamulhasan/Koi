...
import { useListLocations, useListVotes, useCastVote, getListVotesQueryKey, getGetMyVoteQueryKey, useGetMyVote, getGetDashboardSummaryQueryKey, getListLocationsQueryKey } from "@/lib/api.js";
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto w-full pb-24 md:pb-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-2 text-lg">Where to next? Cast your vote, spare the group chat.</p>
      </div>
              ...
              >
                <Card 
                  className={cn(
                    ...
                    isMyVote ? "border-primary shadow-[4px_4px_0_hsl(var(--primary))] bg-primary/10" : "border-card-border bg-card/90 shadow-sm hover:border-primary/50",
                    isSelected && !isMyVote && "ring-2 ring-primary ring-offset-2 ring-offset-background"
                  )}
                  onClick={() => !isMyVote && setSelectedLocation(loc.id)}
                >
                  {isLeading && (
                    <div className="absolute top-0 right-0 bg-accent text-accent-foreground text-[10px] font-bold px-3 py-1 rounded-bl-md z-10 flex items-center gap-1 shadow-sm">
