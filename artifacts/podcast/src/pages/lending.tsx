    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto w-full pb-24 md:pb-8">
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-2 text-lg">Keep the receipts. Who owes who.</p>
        </div>
        
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <Button className="bg-destructive text-destructive-foreground hover:bg-destructive/90 rounded-xl h-12 px-6 shadow-lg shadow-destructive/20 gap-2">
              <Plus className="w-5 h-5" /> New Record
            </Button>
          </DialogTrigger>
          <DialogContent className="bg-card border-card-border sm:max-w-[425px] rounded-2xl">
            <DialogHeader>
            ...
            <form onSubmit={handleCreate} className="space-y-4 pt-4">
              <div className="space-y-2">
                ...
                <Select value={borrowerId} onValueChange={setBorrowerId}>
                  <SelectTrigger className="bg-input border-border h-12 rounded-xl">
              ...
              <Button type="submit" disabled={createRecord.isPending} className="w-full bg-destructive text-destructive-foreground hover:bg-destructive/90 h-12 rounded-xl text-base mt-2">
      ...
      <div className="space-y-12">
        ...
        <section>
          ...
          {activeRecords.length > 0 ? (
                ...
                {activeRecords.map(record => {
                  ...
                  return (
                    <motion.div key={record.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
                      <Card className="bg-card/50 backdrop-blur-xl border-card-border overflow-hidden">
            ...
            <div className="text-center p-10 bg-card/20 rounded-3xl border border-dashed border-border text-muted-foreground">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 opacity-70">
              {repaidRecords.map(record => (
                <Card key={record.id} className="bg-card/30 border-card-border/50">
